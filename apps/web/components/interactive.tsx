...
/* ═══════════════════════════════════════════════════════════════════════════
   TaglineReveal
   Each word resolves individually, in reading order, as it crosses the trigger.

   The dormant state is a token, not an opacity. The spec asks for 25 to 35 per
   cent, which computes to roughly 2.6:1 against this ground and fails even the
   large text floor. Someone who never scrolls past the trigger only ever sees
   that state, so it has to pass on its own. --color-ink-muted measures 4.8:1.
   ═══════════════════════════════════════════════════════════════════════════ */
...
export function TaglineReveal({ lines }: { lines: readonly string[] }) {
  ...
  /**
   * No React state, for the same reason as the theme toggle: this is a
   * progressive visual treatment, not application state. Driving it through the
   * ref means the server renders every word lit — so the sentence is complete
   * with JavaScript off — and it avoids re-rendering every span on each tick.
   */
  useEffect(() => {
    const el = ref.current;
    if (!el) return;
    ...
    const dim = () => spans.forEach((s) => (s.dataset.lit = 'false'));
    const lightAll = () => spans.forEach((s) => (s.dataset.lit = 'true'));

    dim();
    let timer = 0;

    // Unlit words stay legible by design, so this is not a blank-content risk,
    // but a tagline stuck half dim looks broken. Never leave it there.
    const failsafe = window.setTimeout(lightAll, 3000);
    ...
    const io = new IntersectionObserver(
      ([entry]) => {
        ...
        const step = () => {
          const s = spans[i];
          if (s) s.dataset.lit = 'true';
          i += 1;
          if (i < spans.length) timer = window.setTimeout(step, 70);
        };
  ...
  return (
    <p
      ref={ref}
      className="max-w-[var(--container-measure)] text-balance text-4xl font-semibold leading-[1.1] tracking-tight split:text-6xl"
    >
      {words.map((w) => (
        <span key={w.key}>
          {w.br && <br />}
          <span
            data-word
            data-lit="true"
            className="transition-colors duration-700 ease-drawer data-[lit=false]:text-ink-muted data-[lit=true]:text-ink"
          >
            {w.w}
          </span>{' '}
        </span>
